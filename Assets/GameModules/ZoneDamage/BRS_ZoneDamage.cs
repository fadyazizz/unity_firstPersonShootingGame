...
public class BRS_ZoneDamage : MonoBehaviour
{
	...
	public PostProcessProfile outsideZonePPP;
	public PostProcessProfile standardPPP;
	public PostProcessVolume CamPPB;
	...
	void OnTriggerExit(Collider col)
	{
		...
		if (col.transform.name == playername)
		{
			...
			safe.enabled = false;
			danger.enabled = true;
			AudioManager.instance.Play("ZoneOut");
		}
		else if (col.CompareTag("Hero") )
        {
			Debug.Log("hero" + col.gameObject.name);
			Hero h = col.GetComponent<Hero>();
			h.health = 0;
        }else if (col.CompareTag("Champion"))
        {
			Debug.Log("champion" + col.gameObject.name);
			Champion c = col.GetComponent<Champion>();
			c.health = 0;
        }
	}
	...
	void OnTriggerEnter(Collider col)
	{
		...
		if (col.transform.name == playername)
		{
			AudioManager.instance.Play("ZoneIn");
	...
	void DamagePlayer()
	{
		...
		if (!inZone)
		{
			...
			GameManager.inst.TakeDamage(TickDamage);
		}
		//else if(DebugHealth)
		//{
